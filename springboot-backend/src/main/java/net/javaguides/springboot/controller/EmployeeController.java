...
public class EmployeeController {
		...
		try {
			...
			if (authenticate.isAuthenticated()) {
				String jwtToken = jwt.generateToken(c.getUname());
				  System.out.println("Login successful for user: " + c.getUname());
				return new ResponseEntity<>(jwtToken, HttpStatus.OK);
			}
			else {
	            System.out.println("Invalid credentials for user: " + c.getUname());
	...
	public ResponseEntity<Employee> updateEmployee(@PathVariable Long id, @RequestBody Employee employeeDetails){
		...
		employee.setCourses(employeeDetails.getCourses());
